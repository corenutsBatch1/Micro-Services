...
public class UserService {
	...
	public User userLogin(User user) {
		var userinfo = userRepository.findByEmailAndPassword(user.getEmail(), user.getPassword());
	...
	public boolean editUser(User user) {
		var editUser = userRepository.findByEmail(user.getEmail());
		if (editUser != null) {
	...
	public User updateUser(User user) {
		var user2 = userRepository.findById(user.getId()).orElse(null);
		if (user2 != null) {
	...
	public List<User> fetchAllUsers() {
		return userRepository.findAll().stream().filter(e -> e.getRole().equals(Role.USER))
